...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            fake_noise = sample_noise(batch_size, noise_size).to(device)
            fake_images = G(fake_noise).reshape(batch_size, input_channels, img_size, img_size)
            ## Calculate discriminator's prediction on real and fake data
            logits_real = D(real_images).reshape(-1)
            logits_fake = D(fake_images).reshape(-1)
            ...
            # Generator step
            ## Zero gradients in generator
            G_solver.zero_grad()
            ## Re-sample noise and generate fake data
            fake_images = G(fake_noise).reshape(batch_size, input_channels, img_size, img_size)
            ## Calculate discriminator's prediction on the fake data
            logits_fake = D(fake_images)
