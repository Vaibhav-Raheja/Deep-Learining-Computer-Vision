...
class Agent():
    ...
    def get_action(self, state):
        if np.random.rand() <= self.epsilon:
            ...
            with torch.no_grad():
                ...
                a = self.policy_net(state).max(1)[1].item()
                a = torch.tensor(a)
        return a
    ...
    def train_policy_net(self, frame):
        ...
        curr_state_actions = self.policy_net(states) # (batch_size, action_size)
        ...
        next_state_actions = self.target_net(next_states).detach() # (batch_size, action_size)

        # Find Q-value of action at next state from target net
        next_state_values = next_state_actions.gather(1, next_actions.unsqueeze(1)).squeeze(1) * mask.float()
